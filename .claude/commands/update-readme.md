...
4. Add one row per topic to the knowledge table headed by `concept.conventions.knowledge_table` (read from seed.md): `| Item | one-sentence description under 25 words ending in its significance |`. Do **not** duplicate existing rows; preserve all existing content exactly.
